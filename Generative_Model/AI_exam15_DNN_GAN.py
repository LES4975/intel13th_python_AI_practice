...
generator.add(LeakyReLU(alpha=0.01)) # LeakyReLU의 기울기를 0.01로 설정
generator.add(Dense(784, activation='tanh')) # 활성화 함수로 tanh 사용(-1<x<1)
generator.add(Reshape(img_shape))
generator.summary()

# 판별자(Discriminator)
lrelu = LeakyReLU(alpha=0.01) # 아예 LeakyReLU 함수를 변수로 선언(오타 날까봐 그런 듯...)
...
for epoch in range(epochs):
    ...
    if epoch % 5 == 0: # 이 때는 판별자만 학습시키기(학습 속도의 균형이 깨지지 않도록... 판별자만 너무 학습을 잘할까봐...)
